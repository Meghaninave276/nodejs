...
const server = http.createServer((req,res)=>{
    const date=new Date();
    const log=`client request on http://localhost:4600${req.url} by ${req.method} at ${date.getDate()}/${date.getMonth()}/${date.getFullYear()}| ${date.getHours()}:${date.getMinutes()}:${date.getSeconds()} from this IP: ${req.socket.remoteAddress}\n`;
